...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _scanDir() async {
    ...
    final bmarks = await loadAllBookmarks();
    if (mounted) setState(() {
      _bookmarks = bmarks; _loading = false;
  ...
  Future<void> _openBook(Map<String, String> book) async {
    ...
    try {
      ...
      await Navigator.push(context, MaterialPageRoute(
        builder: (_) => ReaderScreen(book: book, tokens: parsed.tokens, startIndex: bm?.wordIndex ?? 0)));
      await _load();
    } catch (e) {
  ...
  Future<void> _jumpTo(Map<String, String> book) async {
    ...
    final result = await showDialog<String>(context: context,
      builder: (_) => AlertDialog(
        ...
        content: TextField(controller: ctrl,
          ...
          decoration: const InputDecoration(hintText: '14.0',
    ...
    try {
      ...
      await saveBookmark(Bookmark(bookId: book['book_id']!, title: book['title']!,
        filepath: book['filepath']!, wordIndex: wi, totalWords: total, progressPct: pct.clamp(0, 100)));
      await _load();
  ...
  Future<void> _searchBook(Map<String, String> book) async {
    ...
    final query = await showDialog<String>(context: context,
      builder: (_) => AlertDialog(
        ...
        content: TextField(controller: ctrl, autofocus: true,
          style: const TextStyle(color: Colors.white),
          decoration: const InputDecoration(hintText: 'phrase to find…',
            hintStyle: TextStyle(color: Colors.grey),
            enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: Colors.grey)))),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
    ...
    try {
      ...
      final tokenIndex = await Navigator.push<int>(context, MaterialPageRoute(
        builder: (_) => SearchScreen(book: book, tokens: parsed.tokens, query: query)));
      if (tokenIndex != null) {
        final total = countWords(parsed.tokens);
        final wb = parsed.tokens.sublist(0, tokenIndex).where((t) => t != paraMarker).length;
        final pct = wb / total.clamp(1, 999999) * 100;
        await saveBookmark(Bookmark(bookId: book['book_id']!, title: book['title']!,
          filepath: book['filepath']!, wordIndex: tokenIndex, totalWords: total, progressPct: pct));
        await _load();
        if (mounted) setState(() => _status = 'Position set to ${pct.toStringAsFixed(1)}%');
      }
    } catch (e) {
  ...
  void _showOptions(Map<String, String> book, int index) {
    showModalBottomSheet(context: context, backgroundColor: const Color(0xFF1a1a2e),
      builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ListTile(leading: const Icon(Icons.percent, color: Colors.white),
          title: const Text('Jump to %', style: TextStyle(color: Colors.white)),
          onTap: () { Navigator.pop(context); _jumpTo(book); }),
        ListTile(leading: const Icon(Icons.search, color: Colors.white),
          title: const Text('Search', style: TextStyle(color: Colors.white)),
  ...
  Widget _buildTile(int i) {
    ...
    final last = bm?.lastOpened.substring(0, 10) ?? '—';
      ...
      child: Container(
        ...
        child: Row(children: [
          Expanded(child: Text(book['title'] ?? '?',
            style: TextStyle(color: sel ? const Color(0xFFEBEBEB) : const Color(0xFFBEBEBE),
              fontWeight: sel ? FontWeight.bold : FontWeight.normal, fontSize: 15),
            overflow: TextOverflow.ellipsis)),
          const SizedBox(width: 8),
          SizedBox(width: 80, child: LinearProgressIndicator(value: pct / 100,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF1a1a2e),
        title: const Text('RSVP Reader', style: TextStyle(color: Color(0xFF00b4d8))),
        actions: [IconButton(icon: const Icon(Icons.folder_open, color: Colors.white),
          tooltip: 'Scan directory', onPressed: _loading ? null : _scanDir)],
      ),
      body: Column(children: [
        Expanded(child: _library.isEmpty
          ? const Center(child: Text('Tap the folder icon to scan for EPUBs',
              style: TextStyle(color: Colors.grey)))
          : ListView.builder(itemCount: _library.length, itemBuilder: (_, i) => _buildTile(i))),
        if (_loading) const LinearProgressIndicator(backgroundColor: Colors.transparent,
          valueColor: AlwaysStoppedAnimation(Color(0xFF00b4d8))),
        if (_status.isNotEmpty)
          Container(width: double.infinity,
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 6),
            color: const Color(0xFF1a1a2e),
            child: Text(_status, style: const TextStyle(color: Colors.grey, fontSize: 12))),
          ...
          SafeArea(child: Padding(padding: const EdgeInsets.all(12),
            child: SizedBox(width: double.infinity,
              child: ElevatedButton(
                ...
                onPressed: _loading ? null : () => _openBook(_library[_selectedIndex]),
                child: Text('Open: ${_library[_selectedIndex]['title'] ?? ''}',
                  overflow: TextOverflow.ellipsis))))),
      ]),
    );
  }
