...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _scanDir() async {
    ...
    for (final book in found) {
      if (!existing.contains(book['book_id'])) {
        _library.add(book);
        added++;
      }
    }
  ...
  Future<void> _openBook(Map<String, String> book) async {
    setState(() { _loading = true; _status = 'Loading ${book['title']}…'; });
    try {
    ...
    try {
      ...
      for (var i = 0; i < parsed.tokens.length; i++) {
        if (parsed.tokens[i] != paraMarker) {
          if (wordCount >= target) { wordIndex = i; break; }
          wordCount++;
        }
      }
      await saveBookmark(Bookmark(
        ...
        wordIndex: wordIndex,
        totalWords: total,
        progressPct: pct.clamp(0, 100),
      ));
      await _load();
      setState(() { _status = 'Position set to ${pct.toStringAsFixed(1)}%'; });
    } catch (e) {
      setState(() { _status = 'Error: $e'; });
    } finally {
      if (mounted) setState(() { _loading = false; });
  ...
  Future<void> _searchBook(Map<String, String> book) async {
    ...
    try {
      final parsed = await parseEpub(book['filepath']!);
      if (!mounted) return;
      final tokenIndex = await Navigator.push<int>(
        context,
        MaterialPageRoute(
      ...
      if (tokenIndex != null) {
        final total = countWords(parsed.tokens);
        final wordsBefore = parsed.tokens
            .sublist(0, tokenIndex)
            .where((t) => t != paraMarker)
            .length;
        final pct = wordsBefore / total.clamp(1, 999999) * 100;
        await saveBookmark(Bookmark(
          bookId: book['book_id']!,
          title: book['title']!,
          filepath: book['filepath']!,
  ...
  Future<void> _removeBook(int index) async {
    ...
    await saveLibrary(_library);
    await _load();
    setState(() { _status = 'Removed ${book['title']}'; });
  }

  void _showBookOptions(Map<String, String> book, int index) {
  ...
  Widget _buildBookTile(int index) {
    ...
    return GestureDetector(
      ...
      onLongPress: () => _showBookOptions(book, index),
      child: Container(
        ...
        children: [
          Expanded(
            child: _library.isEmpty
                ? const Center(
                    child: Text(
                      'Tap the folder icon to scan for EPUBs',
                      style: TextStyle(color: Colors.grey),
                    ),
                  )
                : ListView.builder(
                    itemCount: _library.length,
                    itemBuilder: (_, i) => _buildBookTile(i),
                  ),
          ),
          if (_loading)
              ...
              child: Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: _loading
                        ? null
                        : () => _openBook(_library[_selectedIndex]),
                    child: Text(
